...
void GS(point GS_IN points[1], inout TriangleStream<PS_IN> output)
{
	...
	float3 aPos = input.aPos;
	float3 bPos = input.bPos;
	float3 cPos = input.cPos;

	float aLen = distance(bPos, cPos);
	float bLen = distance(cPos, aPos);
	float cLen = distance(aPos, bPos);
	float3 i = (aLen * aPos + bLen * bPos + cLen * cPos) / (aLen + bLen + cLen);
	...
	PS_IN v0, v1, v2;
	v0.pos = mul(float4(aPos + ai * (1 - radius), 1), tWVP);
	v0.col = input.aCol;
	v1.pos = mul(float4(bPos + bi * (1 - radius), 1), tWVP);
	v1.col = input.bCol;
	v2.pos = mul(float4(cPos + ci * (1 - radius), 1), tWVP);
	v2.col = input.cCol;
